...
const Filters = () => {
  ...
  const fetchShops = async () => {
    try {
      await axios
        .post("http://localhost:8080/api/shops/filters", {
          areaFilter,
          categoryFilter,
          openCloseFilter,
        })
      ...
      <div>
        <Grid container>
          <Grid item lg={4} md={4} sm={6} xs={12}>
            ...
            >
              <Typography
                style={{ fontSize: 18, marginRight: 10, marginTop: 4 }}
              >
                Category:
              </Typography>
              <Select
              ...
              >
                ...
                {categories.map((cat) => {
                  return <MenuItem value={cat.toLowerCase()}>{cat}</MenuItem>;
                })}
              </Select>
            </div>
          </Grid>
          <Grid item lg={4} md={4} sm={6} xs={12}>
            ...
            >
              ...
              >
                <MenuItem value={""}>All</MenuItem>
                {areas.map((ara) => {
                  return <MenuItem value={ara.toLowerCase()}>{ara}</MenuItem>;
                })}
              </Select>
            </div>
          </Grid>
